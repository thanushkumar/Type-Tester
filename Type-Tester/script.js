...
function initTimer() {
  if (timeLeft > 0) {
    ...
    // Capture values and submit form when timer ends
    const cpm = cpmTag.innerText;
    const wpm = wpmTag.innerText;
    const errors = errorTag.innerText;
    // Get username from input field

    const form = document.getElementById('stats-form');
    form.elements['cpm'].value = cpm;
    form.elements['wpm'].value = wpm;
    form.elements['errors'].value = errors;
    form.submit();
  }
}
